...
var macro2 = 
    Sequence(
        ...
        Var("i", 0),
        Var("H2", 
            (string title) => Get<int>("i", 
                i => Sequence(Update("i", i + 1), Text($"<h2>{i}. {title}</h2>")))),
        Channel(1),
        Get<Func<string, Macro>>("H2", H2 => H2("First Section")).Bind(
            section1 => Get<Func<string, Macro>>("H2", H2 => H2("Second Section")).Bind(
                section2 => Get<Func<string, Macro>>("H2", H2 => H2("First Section")).Bind(
                    conclusion => Sequence(
                        WriteLine(section1, section2, conclusion),
                        Read(1).Bind(text => Text($"<HTML>\n{text}</HTML>"))))))
    );

var context = new MacroContext();

Console.WriteLine(macro(ref context));
Console.WriteLine();

context.Reset();

Console.WriteLine(macro2(ref context));
