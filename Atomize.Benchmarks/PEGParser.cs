...
public static class PEGTokenizer
{
    ...
    public static readonly PEGParser RepetitionRule;
    ...
    static PEGTokenizer()
    {
        ...
        RepetitionRule = Bind(
            ...
            predicate => Map(
                ...
                rep => rep == '\0'
                    ...
                    {
                        Label = "REPETITION",
                        Value = $"{predicate.Value}{rep}",
                        Children = new PEGToken[] { predicate }
                    }));

        SequenceRule = Map(
            SeparatedBy(RepetitionRule, Atom(' ')),
            tokens => new PEGToken
            {
                Label = "SEQUENCE",
        ...
        ChoiceRule = Map(
            SeparatedBy(SequenceRule, Whitespaced(Atom('|'))),
            tokens => new PEGToken
            {
                Label = "CHOICE",
                Value = $"({string.Join("|", tokens.Select(token => token.Value))})",
    ...
    private static Parser<T> Whitespaced<T>(Parser<T> parser) =>
        Island(Text.Whitespace, parser, Text.Whitespace);
}
