...
public static class PEGTokenizer
{
    ...
    static PEGTokenizer()
    {
        AtomRule = Handle(
            Map(
                Choice(
                    ...
                    Parse.Text.RegularExpression,
                    Parse.Text.SingleQuotedString),
            ...
            _ => Parse.SurroundedBy.Parentheses(ChoiceRule!));
        ...
        ChoiceRule = Map(
            Split(SequenceRule, Parse.SurroundedBy.Whitespace(Token('|'))),
            tokens => new PEGToken
            {
                Label = "CHOICE",
                Value = $"({string.Join("|", tokens.Select(token => token.Value))})",
        ...
        RuleRule = Bind(
            Parse.Text.Identifier,
            identifier => Bind(
                Ignore(Parse.SurroundedBy.Whitespace(Token(':'))),
                _ => Bind(
                    ChoiceRule,
                    choice => Map(
                        Ignore(Parse.SurroundedBy.Whitespace(Token(';'))),
