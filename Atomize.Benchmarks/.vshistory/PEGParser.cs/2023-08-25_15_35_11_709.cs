...
public static class PEGTokenizer
{
    ...
    static PEGTokenizer()
    {
        ...
        ChoiceRule = Map(
            Split(
                SequenceRule, 
                Island(Token(Pattern.Text.Whitespace), Token('|'), Token(Pattern.Text.Whitespace))),
            tokens => new PEGToken
            {
                Label = "CHOICE",
                Value = $"{string.Join("| ", tokens.Select(token => token.Value))}",
        ...
        RuleRule = Bind(
            Token(Pattern.Text.Identifier),
            identifier => Bind(
                Ignore(Island(Token(Pattern.Text.Whitespace), Token(':'), Token(Pattern.Text.Whitespace))),
                _ => Map(
                    ChoiceRule,
                    choice =>
                    new PEGToken
        ...
        GrammarRule = Bind(
            Minimum(
                ...
                Bind(
                    RuleRule,
                    rule =>
                        Map(Ignore(Island(Token(Pattern.Text.Whitespace), Token(';'), Token(Pattern.Text.Whitespace))),
                    RuleRule =
            ))
    }
}
