...
public static class PEGTokenizer
{
    ...
    static PEGTokenizer()
    {
        AtomRule = Handle(
            Map(
                Choice(
                    ...
                    Pattern.Text.RegularExpression,
                    Pattern.Text.SingleQuotedString),
            ...
            _ => Pattern.SurroundedBy.Parentheses(ChoiceRule!));
        ...
        ChoiceRule = Map(
            Split(SequenceRule, Pattern.SurroundedBy.Whitespace(Token('|'))),
            tokens => new PEGToken
            {
                Label = "CHOICE",
                Value = $"({string.Join("|", tokens.Select(token => token.Value))})",
        ...
        RuleRule = Bind(
            Pattern.Text.Identifier,
            identifier => Bind(
                Ignore(Pattern.SurroundedBy.Whitespace(Token(':'))),
                _ => Bind(
                    ChoiceRule,
                    choice => Map(
                        Ignore(Pattern.SurroundedBy.Whitespace(Token(';'))),
