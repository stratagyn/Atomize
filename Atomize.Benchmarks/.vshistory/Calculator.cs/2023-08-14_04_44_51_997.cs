...
public static partial class BasicArithmeticCalculator
{
    ...
    private static IParseResult<double> Additive1(TokenReader reader) =>
        (_Additive ??= 
    ...
    {
        _Additive ??= from vars in Fallback(Multiplicative, Literal(PlusMinus), Additive)
                      select vars.Item2.Length == 0 
                        ? vars.Item1 
                        : vars.Item2.Span[0] == '+' 
                            ? vars.Item1 + vars.Item3
                            : vars.Item1 - vars.Item3;

        var test = from open in Ignore<int>('(')
                   from lex in Literal(Pattern.Char.LowercaseIdentifier)
                   from close in Ignore<int>(')')
                   select lex;

        var test2 = Ignore<int>('(').SelectMany(Literal('0'), (_, c) => (int)c);



        return _Additive(reader);
    }
