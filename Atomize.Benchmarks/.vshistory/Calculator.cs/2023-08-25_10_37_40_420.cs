...
public static class BasicArithmeticCalculatorTokenizer
{
        ...
        Bind(
            ...
            sign => Bind(Choose(
                Map(
                    ...
                    n => new AST("INTEGER", double.Parse(n.Span))),
                Island(Token('('), Additive, Token(')'))),
                ast => Succeed(sign.Length > 0 ? new AST("OP", sign.Value, new List<AST>() { ast.Value }) : ast)));

    private static Parser<AST>? _Additive;
    private static Parser<AST>? _Multiplicative;
    private static Parser<AST>? _Exponential;

    public static IParseResult<AST> Parse(string expression) => Additive(new(expression, true));

    private static IParseResult<AST> Additive(TextScanner scanner)
    {
        return (_Additive ??= Map(
            TryBind(
                Multiplicative,
    ...
    private static IParseResult<AST> Multiplicative(TextScanner scanner)
    {
        return (_Multiplicative ??= Map(
            TryBind(
                Exponential,
    ...
    private static IParseResult<AST> Exponential(TextScanner scanner)
    {
        return (_Exponential ??= Map(
        Split(Atom!, Token('^')),
        vars =>
