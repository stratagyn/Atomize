...
{
    private readonly Parser<double> Additive = Map(
            TryBind(
                Multiplicative,
                x => Bind(
                    Choose(Token('+'), Token('-')),
    ...
    private readonly Parser<double> Multiplicative = Map(
            TryBind(
                Exponential,
                x => Bind(
                    Choose(Token('*'), Token('/')),
    ...
    private readonly Parser<double> Exponential = Map(
        Split(Atom, Token('^')),
        vars =>
        {
    ...
    private readonly Parser<double> Atom =
        Bind(
