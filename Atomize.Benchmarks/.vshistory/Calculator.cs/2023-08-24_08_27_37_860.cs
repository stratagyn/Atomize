...
public static class BasicArithmeticCalculator
{
    ...
    private static readonly Parser<double>? _Additive;
    private static readonly Parser<double>? _Multiplicative;
    private static readonly Parser<double>? _Exponential;

    public static IParseResult<double> Parse(string expression) => Additive(new(expression, true));

    private static Parser<double> Additive(TextScanner scanner)
    {
    ...
    private static Parser<double> Multiplicative(TextScanner scanner)
    {
    ...
    private static Parser<double> Exponential = Map(
        Split(Atom!, Token('^')),
        vars =>
        {
