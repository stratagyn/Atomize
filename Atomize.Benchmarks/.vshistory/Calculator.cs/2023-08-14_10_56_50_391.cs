...
public static partial class BasicArithmeticCalculator
{
    private static Parser<double> Atom =
        Optional(Token(PlusMinusRegex())).Then(
            sign =>
                Choose(
                    Token(Pattern.Text.Integer).As(n => double.Parse(n.Span)),
                    Island(Token('('), Additive, Token(')'))).As(
                    n => sign.Value.Length > 0 && sign.Value.Span[0] == '-' ? -n : n));
    ...
    private static IParseResult<double> Additive(in Scanner reader) =>
        (_Additive ??= 
            Fallback(Multiplicative, Token(PlusMinus), Additive).As(
                vars =>
                {
    ...
    private static IParseResult<double> Multiplicative(in Scanner reader) =>
        (_Multiplicative ??=
            Fallback(Exponential, Token(MulDiv), Multiplicative)
            .As(vars =>
            {
                if (vars.Item2.Length == 0)
                    return vars.Item1;
    ...
    private static IParseResult<double> Exponential(in Scanner reader) =>
        (_Exponential ??=
            Sequence(
                Atom, 
                Parser.Handle(
                    SeparatedBy(Token('^'), Exponential), 
                    fail => new Lexeme<IList<double>>(fail.Offset, 0, Array.Empty<double>())))
                .As(vars =>
                {
                    var (b, pows) = vars;
