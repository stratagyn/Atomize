...
var parser = from x in Parse.Text.Integer
             from op in Parse.Choice(Parse.Token('+'), Parse.Token('-'))
             from y in Parse.Text.Integer
             where x.Length < 3 && y.Length < 3
             select int.Parse(x.Span) ;
