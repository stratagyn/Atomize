...
if (!result.IsToken)
{
    var start = Math.Min(0, result.Offset - 5);
    var end = Math.Max(peggy.Length, result.Offset + 5);

    var before = peggy[start..result.Offset];
    var after = peggy[(result.Offset + 1)..end];
    Console.WriteLine($"{before}{peggy[result.Offset]}{after}");
    Console.WriteLine($"{string.Join("", Enumerable.Range(0, before.Length).Select(_ => ' '))})
