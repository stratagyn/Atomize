...
public static partial class AtomizeTests
{
    ...
    public class EOTTests
    {
        ...
        public EOTTests() => _reader = new TokenReader(TestText);

        [Fact]
        public void EOT_Matches_AtEnd_Is_Successful()
        {
            _reader.Advance(TestText.Length);

            var actual = EOT(in _reader).IsSuccess;
            Assert.True(actual);
        }

        [Fact]
        public void EOT_Matches_NotAtEnd_Is_Failure()
        {
            var actual = EOT(in _reader).IsSuccess;
            Assert.False(actual);
        }
