...
public static partial class AtomizeTests
{
    ...
    public class HandleTests
    {
        ...
        {
            var parser = Handle(Token('a'), err => new EmptyToken<char>(err.Offset));
            var expected = (int)'a';
        ...
        {
            var parser = Handle(Token('a'), a => (int)'a');
            var expected = _reader.Offset + 1;
        ...
        [Fact]
        public void Handle_NonMatch_Is_Failure()
        {
            var parser = Handle(Token('A'), a => (int)a);
            var actual = parser(_reader).IsToken;
        ...
        [Fact]
        public void Handle_NonMatch_AfterMatch_DoesNot_AdvanceOffset()
        {
            var expected = _reader.Offset;

            _ = Handle(Token('a'), a => Handle(Token('B'), _ => Succeed(a)))(_reader);
