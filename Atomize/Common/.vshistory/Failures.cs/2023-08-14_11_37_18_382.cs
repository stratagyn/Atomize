...
internal static class Failure
{
    public static class DidNotExpect
    {
        ...
        public static Failure<T> Char<T>(Scanner scanner, char token, int at, int start) =>
            Undo<T>(scanner, at, start, $"Expected not to match {{'{token}'}}.");
        ...
        public static Failure<T> Regex<T>(Scanner scanner, Regex token, int at, int start) =>
            Undo<T>(scanner, at, start, $"Expected not to match {{/{token}/}}.");
        ...
        public static Failure<T> Text<T>(Scanner scanner, ReadOnlySpan<char> token, int at, int start) =>
            Undo<T>(scanner, at, start, $"Expected not to match {{\"{new string(token)}\"}}.");
    }
    ...
    public static class Expected
    {
        ...
        public static Failure<T> Char<T>(Scanner scanner, char token, int at, int start) =>
            Undo<T>(scanner, at, start, $"Expected to match {{'{token}'}}.");

        public static Failure<T> Char<T>(Scanner scanner, char[] tokens, int at, int start) =>
            Undo<T>(scanner, at, start, $"Expected to match one of {{{Utilities.Join(tokens)}}}.");
        ...
        public static Failure<T> EnoughMatchingTokens<T>(Scanner scanner, int n, char token, int at, int start) =>
            Undo<T>(scanner, at, start, $"Failed to match '{token}' {n} times.");

        public static Failure<T> EnoughMatchingTokens<T>(Scanner scanner, int n, string token, int at, int start) =>
            Undo<T>(scanner, at, start, $"Failed to match \"{token}\" {n} times.");

        public static Failure<T> EnoughMatchingTokens<T>(Scanner scanner, int n, Regex token, int at, int start) =>
            Undo<T>(scanner, at, start, $"Failed to match /{token}/ {n} times.");
        ...
        public static Failure<T> Regex<T>(Scanner scanner, Regex[] tokens, int at, int start) =>
            Undo<T>(scanner, at, start, $"Expected to match one of {{{Utilities.Join(tokens)}}}.");
        ...
        public static Failure<T> Text<T>(Scanner scanner, ReadOnlySpan<char> token, int at, int start) =>
            Undo<T>(scanner, at, start, $"Expected to match {{\"{new(token)}\"}}.");

        public static Failure<T> Text<T>(Scanner scanner, string[] tokens, int at, int start) =>
            Undo<T>(scanner, at, start, $"Expected to match one of {{{Utilities.Join(tokens)}}}.");
        ...
        public static Failure<T> ToPass<T>(Scanner scanner, char token, int at, int start) =>
            Undo<T>(scanner, at, start, $"Expected {token} to pass predicate.");

        public static Failure<T> ToPass<T>(string token, int at) =>
            new(at, $"Expected {token} to pass predicate.");

        public static Failure<T> ToPass<T>(Scanner scanner, string token, int at, int start) =>
            Undo<T>(scanner, at, start, $"Expected {token} to pass predicate.");

        public static Failure<T> ToPass<T>(T token, int at) =>
            new(at, $"Expected {token} to pass predicate.");

        public static Failure<T> ToPass<T>(Scanner scanner, T token, int at, int start) =>
            Undo<T>(scanner, at, start, $"Expected {token} to pass predicate.");
