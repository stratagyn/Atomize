...
internal static class Failure
{
   ...
   public static class Expected
   {
      ...
      public static Failure<T> Char<T>(char[] parsers, int at) =>
         new(
            at, 
            $"Expected to match one of {{{string.Join(", ", parsers.Select(c => $"'{c}'"))}}}.");
      ...
      {
         var patterns = parsers.Select(parser => $"/{parser.ToString().Replace("/", "\\/")}/");

         return new(at, $"Expected to match one of {{{string.Join(", ", patterns)}}}.");
      }
      ...
      public static Failure<T> Text<T>(string[] parsers, int at) =>
         new(
            at, 
            $"Expected to match one of {{{string.Join(", ", parsers.Select(s => $"\"{s}\""))}}}.");
