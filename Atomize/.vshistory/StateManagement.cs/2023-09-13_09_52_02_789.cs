...
public static partial class Parse
{
    public static Parser<U> Bind<T, U>(Parser<T> parser, Func<T, Parser<U>> bind) =>
        (TextScanner scanner) =>
        {
            ...
            if (!result.IsToken)
                return Undo<U>(scanner, result.Offset, at, result.Error);

            var boundResult = bind(result.Value!)(scanner);

            if (!boundResult.IsToken)
                return Undo<U>(scanner, boundResult.Offset, at, boundResult.Error);
    ...
    public static Parser<T> Handle<T>(Parser<T> parser, Parser<T> handle) =>
        (TextScanner scanner) =>
        {
            ...
            if (!handleResult.IsToken)
                return Undo<T>(scanner, handleResult.Offset, at, handleResult.Error);
    ...
    public static Parser<U> If<T, U>(Parser<T> parser, Func<T, Parser<U>> bind, Parser<U> handle) =>
        (TextScanner scanner) =>
        {
            ...
            if (!result.IsToken)
            {
                ...
                if (!handleResult.IsToken)
                    return Undo<U>(scanner, handleResult.Offset, at, handleResult.Error);
    ...
    public static Parser<(T, IParseResult<U>)> TryBind<T, U>(Parser<T> parser, Func<T, Parser<U>> bind) =>
        (TextScanner scanner) =>
        {
            ...
            if (!firstResult.IsToken)
                return Undo<(T, IParseResult<U>)>(scanner, firstResult.Offset, at, firstResult.Error);
