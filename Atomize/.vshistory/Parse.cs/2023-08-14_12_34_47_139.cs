    ...
    public static Parser<(T, U?)> ThenTry<T, U>(this Parser<T> parser, Func<IParseResult<T>, Parser<U>> bind) =>
        (Scanner scanner) =>
        {
            ...
            if (!firstResult.IsToken)
                return Undo<(T, U?)>(scanner, firstResult.Offset, startingAt, firstResult.Conflict);

            var secondResult = bind(firstResult)(scanner);

            if (!secondResult.IsToken)
            {
                scanner.Offset = startingAt + firstResult.Length;

                return new Lexeme<(T, U?)>(startingAt, firstResult.Length, (firstResult.Value!, default));
            }

            return new Lexeme<(T, U?)>(startingAt, firstResult.Length + secondResult.Length, )
        };
}