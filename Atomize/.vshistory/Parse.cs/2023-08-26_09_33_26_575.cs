...
public delegate IParseResult<T> Parse<T>(TextScanner scanner);

public static partial class Parse
{
    public static Parse<U> Bind<T, U>(Parse<T> parser, Func<IParseResult<T>, Parse<U>> bind) =>
        (TextScanner scanner) =>
        {
    ...
    public static Parse<T> Handle<T>(Parse<T> parser, Func<IParseResult<T>, Parse<T>> handle) =>
        (TextScanner scanner) =>
        {
            var at = scanner.Offset;
            var result = parser(scanner);
    ...
    public static Parse<U> Map<T, U>(Parse<T> parser, Func<T, U> map) =>
        (TextScanner scanner) =>
        {
            var at = scanner.Offset;
            var result = parser(scanner);
    ...
    public static Parse<(T, IParseResult<U>)> TryBind<T, U>(Parse<T> parser, Func<IParseResult<T>, Parse<U>> bind) =>
        (TextScanner scanner) =>
        {
            var at = scanner.Offset;
