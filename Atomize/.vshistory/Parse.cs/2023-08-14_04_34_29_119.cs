...
public static partial class Parser
{
    ...
    public static Parser<R> SelectMany<T, U, R>(Parser<T> first, Parser<U> second, Func<(T, U)> ) =>
        (TokenReader reader) =>
        {
            ...
            if (!firstResult.IsToken)
                return Undo<(T, U)>(reader, firstResult.Offset, at, firstResult.Conflict);

            var secondResult = second(reader);

            if (!secondResult.IsToken)
                return Undo<(T, U)>(reader, secondResult.Offset, at, secondResult.Conflict);

            return new Token<(T, U)>(
                at, reader.Offset - at, (firstResult.SemanticValue!, secondResult.SemanticValue!));
        };
