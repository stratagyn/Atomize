...
public static partial class Parser
{
    ...
    public static Parser<R> SelectMany<T, U, R>(this Parser<T> first, Parser<U> second, Func<T, U, R> map) =>
        (TokenReader reader) =>
        {
    ...
    public static Parser<R> SelectMany<T, U, V, R>(this Parser<T> first, Parser<U> second, Parser<V> third, Func<T, U, V, R> map) =>
        (TokenReader reader) =>
        {
            var at = reader.Offset;
            var firstResult = first(reader);
            ...
            if (!thirdResult.IsToken)
                return Undo<R>(reader, thirdResult.Offset, at, thirdResult.Conflict);

            return new Token<R>(
                at,
                reader.Offset - at,
                map(firstResult.SemanticValue!, secondResult.SemanticValue!, thirdResult.SemanticValue!));
        };
