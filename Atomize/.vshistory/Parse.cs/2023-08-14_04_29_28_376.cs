...
public static partial class Parser
{
    ...
    public static Parser<U> Select<T, U>(this Parser<T> parser, Func<T, U> bind) =>
        (TokenReader reader) =>
        {
            var startingOffset = reader.Offset;
            var result = parser(reader);
            ...
            return new Token<U>(result.Offset, result.Length, bind(result.SemanticValue!));
        };

    public static Parser<U> SelectMany<T, U>(this IEnumerable<Parser<T>> sequence, Func<T, >)
