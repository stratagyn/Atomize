...
namespace Atomize
{
    ...
    public static partial class Parse
    {
        ...
        public static Parser<U> Match<T, U>(Parser<T> parser, Func<T, Parser<U>> bind, Parser<U> handle) =>
            (TextScanner scanner) =>
            {
                var at = scanner.Offset;
                var result = parser(scanner);
                ...
                if (!result.IsToken)
                {
                    ...
                    if (!handleResult.IsToken)
                        return Undo<U>(scanner, handleResult.Offset, at, handleResult.Conflict);
