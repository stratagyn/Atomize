...
public static partial class Parser
{
    ...
    public static IParseResult<char> Literal(TokenReader reader)
    {
        ...
        return new LiteralChar(reader.Offset, token);
    }
    ...
    public static Parser<string> Literal(int n) =>
        (TokenReader reader) =>
        {
            ...
            return new LiteralText(reader.Offset, token);
        };
    ...
    public static Parser<char> Literal(char token) =>
        (TokenReader reader) =>
        {
            ...
            return reader.StartsWith(token)
                ? new LiteralChar(reader.Offset, reader.ReadChar())
    ...
    public static Parser<string> Literal(string token) =>
        (TokenReader reader) =>
        {
            ...
            return reader.StartsWith(token)
                ? new LiteralText(reader.Offset, reader.ReadText(token.Length))
    ...
    public static Parser<string> Literal(Regex token) =>
        (TokenReader reader) =>
            reader.StartsWith(token, out var length)
                ? new LiteralText(reader.Offset, reader.ReadText(length))
