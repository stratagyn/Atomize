...
public static partial class Parser
{
    ...
    public static Parser<U> Bind<T, U>(this Parser<T> parser, Func<IParseResult<T>, Parser<U>> bind) =>
        (Scanner scanner) =>
        {
            var at = scanner.Offset;
            var result = parser(scanner);
    ...
    public static Parser<(T, U?)> TryBind<T, U>(this Parser<T> parser, Func<IParseResult<T>, Parser<U>> bind) =>
        (Scanner scanner) =>
        {
            var at = scanner.Offset;
            ...
            if (!firstResult.IsToken)
                return Undo<(T, U?)>(scanner, firstResult.Offset, at, firstResult.Conflict);
