...
public struct TokenReader
{
    ...
    public readonly bool FollowedBy(string token) =>
    ...
    public readonly bool FollowedBy(Regex token)
    {
        if (Remaining == 0)
    ...
    public readonly bool PrecededBy(char token) =>
        Offset > 0 && Text.Span[Offset - 1] == token;

    public readonly bool PrecededBy(string token) =>
        Offset >= token.Length && MemoryExtensions.Equals(
            Text.Span[(Offset - token.Length)..Offset],
            token,
            StringComparison.Ordinal);

    public readonly bool PrecededBy(Regex token)
    {
        var span = Text.Span;
    ...
    public readonly bool StartsWith(char token) =>
        Offset < Text.Length && Text.Span[Offset] == token;

    public readonly bool StartsWith(char[] tokens)
    {
        if (Remaining == 0)
            return false;
    ...
    public readonly bool StartsWith(string token) =>
        Remaining >= token.Length &&
        MemoryExtensions.Equals(
    ...
    public readonly bool StartsWith(string[] tokens) => StartsWith(tokens, out _);

    public readonly bool StartsWith(string[] tokens, out string match)
    {
        match = "";
    ...
    public readonly bool StartsWith(Regex token) => StartsWith(token, out _);

    public readonly bool StartsWith(Regex token, out int length)
    {
        length = 0;
    ...
    public readonly bool StartsWith(Regex[] tokens) => StartsWith(tokens, out _);

    public readonly bool StartsWith(Regex[] tokens, out int length)
    {
        length = 0;
