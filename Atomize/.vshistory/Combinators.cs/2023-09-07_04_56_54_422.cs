...
public static partial class Parse
{
    ...
    public static Parser<IList<T>> Exactly<T>(int n, Parser<T> parser) =>
        (TextScanner scanner) =>
        {
            ...
            if (matched.Count < n)
                return Undo<IList<T>>(scanner, match.Offset, at, match.Conflict);
    ...
    public static Parser<T> IfFollowedBy<T, F>(Parser<T> parser, Parser<F> assertion) =>
        (TextScanner scanner) =>
        {
            ...
            if (!parsed.IsToken)
                return Undo<T>(scanner, parsed.Offset, at, parsed.Conflict);

            var followedBy = assertion(scanner);

            if (!followedBy.IsToken)
                return Undo<T>(scanner, followedBy.Offset, at, followedBy.Conflict);
    ...
    public static Parser<T> IfPrecededBy<T, P>(Parser<T> parser, Parser<P> assertion) =>
        (TextScanner scanner) =>
        {
            ...
            if (!precededBy.IsToken)
                return Undo<T>(scanner, precededBy.Offset, at, precededBy.Conflict);
    ...
    public static Parser<IList<T>> Join<S, T>(Parser<S> separator, params Parser<T>[] parsers) => (TextScanner scanner) =>
    {
        ...
        if (parsers.Length == 1)
        {
            result = parsers[0](scanner);

            if (!result.IsToken)
                return Undo<IList<T>>(scanner, result.Offset, at, result.Conflict);

            return new Lexeme<IList<T>>(at, scanner.Offset - at, new List<T>() { result.Value! });
        }
        ...
        for (var i = 0; i < parsers.Length - 1; i++)
        {
            ...
            if (!sResult.IsToken)
                return Undo<IList<T>>(scanner, sResult.Offset, at, sResult.Conflict);
        }
    ...
    public static Parser<T> Island<O, T, C>(Parser<O> open, Parser<T> parser, Parser<C> close) =>
        (TextScanner scanner) =>
        {
            ...
            if (!opener.IsToken)
                return Undo<T>(scanner, opener.Offset, at, opener.Conflict);
            ...
            if (!closer.IsToken)
                return Undo<T>(scanner, closer.Offset, at, closer.Conflict);
    ...
    public static Parser<T> NotFollowedBy<T, F>(Parser<T> parser, Parser<F> assertion) =>
        (TextScanner scanner) =>
        {
            ...
            if (!parsed.IsToken)
                return Undo<T>(
                    scanner, parsed.Offset, at, parsed.Conflict);
        ...
        (TextScanner scanner) =>
        {
            ...
            if (!second.IsToken)
                return Undo<(IParseResult<T>, IParseResult<U>)>(
                    ...
                    second.Offset,
                    at,
                    $"{{@ {first.Offset} : {first.Conflict}}} \u2228 {{@ {second.Offset} : {second.Conflict}}}");
