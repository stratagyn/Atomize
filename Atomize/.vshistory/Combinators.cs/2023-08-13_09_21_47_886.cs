...
public static partial class Parser
{
    ...
    public static Parser<IList<T>> Fallback<T>(params Parser<T>[] parsers) =>
        (TokenReader reader) =>
        {
            ...
            var matched = new List<T>() { first.SemanticValue! };
    ...
    public static Parser<IList<T>> Join<T>(Parser<T> separator, params Parser<T>[] tokens) => (TokenReader reader) =>
    {
        if (tokens.Length == 0)
            return new EmptyToken<IList<T>>(reader.Offset);
    ...
    public static Parser<IList<T>> Minimum<T>(int n, Parser<T> token) =>
        (TokenReader reader) =>
        {
            ...
            while((match = token(reader)).IsToken)
                matched.Add(match.SemanticValue!);
    ...
    public static Parser<(T?, T?)> Or<T>(Parser<T> rule1, Parser<T> rule2) => 
        (TokenReader reader) =>
        {
            ...
            return new Token<(T?, T?)>(at, second.Length, (default, second.SemanticValue!));
        };
    ...
    public static Parser<ReadOnlyMemory<char>> Until<T>(Parser<T> token) => 
        (TokenReader reader) =>
        {
            var at = reader.Offset;
            var result = token(reader);
            ...
            while (!result.IsToken && reader.Remaining > 0)
            {
                index++;
                reader.Offset++;
                result = token(reader);
            ...
            if (!result.IsToken)
                return new Token<ReadOnlyMemory<char>>(at, reader.Text.Length - at, reader.ReadToEnd());

            return new Token<ReadOnlyMemory<char>>(at, index - at, reader.ReadText(index - at));
        };
