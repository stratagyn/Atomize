...
public static partial class Parser
{
    ...
    public static Parser<IList<T>> LongestPrefix<T>(params Parser<T>[] parsers) =>
        (TokenReader reader) =>
        {
            ...
            if (parsers.Length == 0)
                return ParseResult.Succeed<IList<T>>(startingOffset, "", Array.Empty<T>());

            var result = parsers[0](reader);

            if (!result.IsToken)
                return Manage.Undo<IList<T>>(reader, result.Offset, startingOffset, result.ReasonForFailure);

            IList<T> matched = new List<T>() { result.Token! };
