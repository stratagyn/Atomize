...
public static partial class Parser
{
    ...
    public static Parser<T> Or<T>(Parser<T> rule1, Parser<T> rule2) => 
        (TokenReader reader) =>
        {
            var at = reader.Offset;

            var result1 = rule1(reader);
            ...
            if (result1.IsSuccess)
            {
                ...
                return new Lexeme(reader.Text[reader.Offset - at], new ParseResult[] { result1, result }, at);
            }
            ...
            if (!result2.IsSuccess)
                return new Failure(
                    $"{result1.ReasonForFailure} => {result2.ReasonForFailure}",
                    at);
