...
public static partial class Parser
{
    ...
    public static Parser<IList<T>> Interleave<T>(Parser<T> separator, params Parser<T>[] tokens) => (TokenReader reader) =>
    {
        ...
        var at = reader.Offset;
        IList<T> matched = new List<T>();
        IParseResult<T> tokenResult;

        for (var i = 0; i < tokens.Length - 1; i++)
        {
            var token = tokens[i];
            tokenResult = token(reader);
            ...
            if (!tokenResult.IsToken)
                return Manage.Undo<IList<T>>(
                    ...
                    tokenResult.Offset,
                    at,
                    tokenResult.ReasonForFailure);

            matched.Add(tokenResult.Token!);

            var separatorResult = separator(reader);

            if (!separatorResult.IsToken)
                return Manage.Undo<IList<T>>(
                    reader,
                    separatorResult.Offset,
                    at,
                    separatorResult.ReasonForFailure);
        }

        tokenResult = tokens[^1](reader);
        ...
        return ParseResult.Succeed(at, reader.Text[at..reader.Offset], matched);
    };
