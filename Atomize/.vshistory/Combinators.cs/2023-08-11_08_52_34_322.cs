...
public static partial class Parser
{
    ...
    public static Parser<IList<T>> Range<T>(int min, int max, Parser<T> token) =>
        ...
        {
            if (min < 0 || max < min)
                return Expected<IList<T>>.ValidRange<IList<T>>(min, max);
            ...
            for (var i = 0; i < max && (match = token(reader)).IsToken; i++)
                matched.Add(match.Token!);

            if (matched.Count < min)
                return Manage.NotEnoughTokensMatched<IList<T>>(
                    reader, min, reader.Offset, startingOffset);

            return ParseResult.Succeed(
                startingOffset,
                reader.Text[startingOffset..reader.Offset],
                matched);
        };
    ...
    public static Parser<T> Satisfies<T>(Parser<T> rule, Predicate<T> cond) =>
        (TokenReader reader) =>
        {
            ...
            if (!result.IsToken)
                return Undo<T>(reader, result.Offset, startingOffset, result.Conflict);

            if (!cond(result.SemanticValue!))
                return Expected.ToPass(result.SemanticValue!, startingOffset);
