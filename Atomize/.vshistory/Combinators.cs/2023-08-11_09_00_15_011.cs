...
public static partial class Parser
{
    ...
    public static Parser<T?> Optional<T>(Parser<T> token) =>
        (TokenReader reader) =>
        {
            ...
            ParseResult<T> match = token(reader);

            if (!match.IsToken)
            {
                reader.Backtrack(reader.Offset - startingOffset);

                return ParseResult.Succeed<T?>(reader.Offset, "", default);
            }

            return ParseResult.Succeed<T?>(startingOffset, match.Match, match.Token);
        };
