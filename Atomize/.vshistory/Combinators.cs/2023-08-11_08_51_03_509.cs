...
public static partial class Parser
{
    ...
    public static Parser<T> Satisfies<T>(Parser<T> rule, Predicate<T> cond) =>
        (TokenReader reader) =>
        {
            ...
            if (!result.IsToken)
                return Manage.Undo<T>(reader, result.Offset, startingOffset, result.ReasonForFailure);

            if (!cond(result.Token!))
                return Expected.True<T>(result.Token!, startingOffset);
