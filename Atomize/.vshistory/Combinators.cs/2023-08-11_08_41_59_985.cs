        ...
        (TokenReader reader) =>
        {
            ...
            if (!result.IsToken)
                return Manage.Undo<IList<T>>(
                    reader, result.Offset, at, result.ReasonForFailure);
            ...
            while (result.IsToken)
            {
                ...
                result = separator(reader);

                if (!result.IsToken)
                {
                    reader.Backtrack(reader.Offset - currentOffset);

                    return ParseResult.Succeed(at, reader.Text[at..currentOffset], matched);
                }
            ...
            reader.Backtrack(result.Length);
