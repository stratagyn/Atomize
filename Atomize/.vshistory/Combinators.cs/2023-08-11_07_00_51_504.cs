...
public static partial class Parser
{
    ...
    public static Parser<IList<T>> Fallback<T>(params Parser<T>[] parsers) =>
        (TokenReader reader) =>
        {
            ...
            IList<T> matched = new List<T>() { firstResult.SemanticValue! };
            ...
            for (var i = 1; i < parsers.Length; i++)
            {
                ...
                if (!result.IsToken)
                {
                    ...
                    return ParseResult.Match<IList<T>>(
                        startingAt,
                        firstResult.Length,
                        UnitArray(firstResult.SemanticValue!));
                }
            ...
            return ParseResult.Match(startingAt, reader.Text[startingAt..reader.Offset], matched);
        };
}
