...
public static partial class Parse
{
    public static Parser<T> Choice<T>(params Parser<T>[] parsers) =>
        (TextScanner scanner) =>
        {
            ...
            foreach (var parser in parsers)
            {
                ...
                errors.Add((result.Offset, result.Error));
            }
    ...
    public static Parser<IList<T>> Exactly<T>(int n, Parser<T> parser) =>
        (TextScanner scanner) =>
        {
            ...
            if (matched.Count < n)
                return Undo<IList<T>>(scanner, match.Offset, at, match.Error);
    ...
    public static Parser<T> IfFollowedBy<T, F>(Parser<T> parser, Parser<F> assertion) =>
        (TextScanner scanner) =>
        {
            ...
            if (!parsed.IsToken)
                return Undo<T>(scanner, parsed.Offset, at, parsed.Error);

            var followedBy = assertion(scanner);

            if (!followedBy.IsToken)
                return Undo<T>(scanner, followedBy.Offset, at, followedBy.Error);
    ...
    public static Parser<T> IfPrecededBy<T, P>(Parser<T> parser, Parser<P> assertion) =>
        (TextScanner scanner) =>
        {
            ...
            if (!precededBy.IsToken)
                return Undo<T>(scanner, precededBy.Offset, at, precededBy.Error);
    ...
    public static Parser<IList<T>> Join<S, T>(Parser<S> separator, params Parser<T>[] parsers) => (TextScanner scanner) =>
    {
        ...
        for (var i = 0; i < parsers.Length - 1; i++)
        {
            ...
            if (!result.IsToken)
                return Undo<IList<T>>(scanner, result.Offset, at, result.Error);
            ...
            if (!sResult.IsToken)
                return Undo<IList<T>>(scanner, sResult.Offset, at, sResult.Error);
        }
    ...
    public static Parser<T> Island<O, T, C>(Parser<O> open, Parser<T> parser, Parser<C> close) =>
        (TextScanner scanner) =>
        {
            ...
            if (!opener.IsToken)
                return Undo<T>(scanner, opener.Offset, at, opener.Error);
            ...
            if (!closer.IsToken)
                return Undo<T>(scanner, closer.Offset, at, closer.Error);
    ...
    public static Parser<T> NotFollowedBy<T, F>(Parser<T> parser, Parser<F> assertion) =>
        (TextScanner scanner) =>
        {
            ...
            if (!parsed.IsToken)
                return Undo<T>(
                    scanner, parsed.Offset, at, parsed.Error);
        ...
        (TextScanner scanner) =>
        {
            ...
            if (!second.IsToken)
                return Undo<(IParseResult<T>, IParseResult<U>)>(
                    ...
                    second.Offset,
                    at,
                    $"{{@ {first.Offset} : {first.Error}}} \u2228 {{@ {second.Offset} : {second.Error}}}");
    ...
    public static Parser<T> Satisfies<T>(Parser<T> parser, Predicate<T> test) =>
        (TextScanner scanner) =>
        {
            ...
            if (!result.IsToken)
                return Undo<T>(scanner, result.Offset, at, result.Error);
