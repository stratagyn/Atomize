...
internal class LiteralCache
{
    ...
    public static Parser<ReadOnlyMemory<char>> Match(string token) =>
        Characters.GetOrAdd(
            ...
            {
                if (reader.Remaining == 0)
                    return DidNotExpect.EndOfText<ReadOnlyMemory<char>>(reader.Offset);

                return reader.StartsWith(token)
                    ? new Character(reader.Offset, reader.Read(token.Length))
    ...
    public static Parser<ReadOnlyMemory<char>> Match(Regex token) =>
        Patterns.GetOrAdd(
            ...
            (TokenReader reader) =>
            {
                ...
                return reader.StartsWith(token, out var length)
                    ? new Text(reader.Offset, reader.Read(length))
